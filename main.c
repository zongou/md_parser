...
MD_NODE *md_find_node(MD_NODE *head, char **heading_path, int num_headings) {
    if (head == NULL) {
        return NULL;
    }

    MD_NODE *current = head;
    // printf("node=%s, heading_path=%s, num_headings=%d\n", current->text, heading_path[0], num_headings);
    while (current) {
        if (strcasecmp(heading_path[0], current->text) == 0) {
            if (num_headings > 1) {
                return md_find_node(current->child, &heading_path[1], num_headings - 1);
            } else {
                return current;
            }
            break;
...
void show_help() {
    printf("Usage: %s [OPTIONS] [SUB_HEADING...] -- [ARGS]\n"
           "Options\n"
